...
#define BCM2835_ADD_BASE 0x3F200000
/* Thông tin driver */
MODULE_LICENSE("GPL");
...
/*on_gpio*/
static void on_gpio(void)
{
	unsigned int* gpiosel=(unsigned int*)((char*)gpio_register + 0x00);
	unsigned int* gpio_on_out=(unsigned int*)((char*)gpio_register + 0x1C);
	*gpiosel=0x01;
	*gpio_on_out=0x01;
}
/*off_gpio*/
static void off_gpio(void)
{
	unsigned int* gpio_off_out=(unsigned int*)((char*)gpio_register + 0x28);
	*gpio_off_out=0x01;
}
/* driver - open*/
...
static int __init DRIVER_GPIO_INIT(void)
{
	...
	/* xu li GPIO init */
	gpio_register=(int*)ioremap(BCM2835_ADD_BASE,PAGE_SIZE);
